...
export default function EditInventoryItemPage({ params }: { params: { id: string } }) {
  ...
  // Fetch inventory item, products, and companies
  useEffect(() => {
    async function fetchData() {
      try {
        setLoading(true)
        const [itemRes, productsRes, companiesRes] = await Promise.all([
          fetch(`/api/inventory/${params.id}`),
          fetch("/api/products"),
          fetch("/api/companies"),
        ])

        if (!itemRes.ok || !productsRes.ok || !companiesRes.ok) {
    ...
    <div className="container mx-auto p-4 md:p-6">
      <Card className="mx-auto max-w-2xl overflow-hidden">
        <CardHeader className="bg-gradient-to-r from-sp-blue/10 to-sp-red/10">
          ...
          <CardDescription>Update the inventory item details.</CardDescription>
        </CardHeader>
